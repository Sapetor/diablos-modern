...
def create_nonlinear_example():
    ...
    # Input: Sine wave
    builder.add_block(
        "Sine", 50, 200, name="input", params={"amplitude": 2.0, "frequency": 0.5, "phase": 0.0}
    )

    # Saturation block (-1 to +1)
    builder.add_block(
        "Saturation", 200, 100, name="sat", params={"upper_limit": 1.0, "lower_limit": -1.0}
    )
    ...
    builder.add_block(
        ...
        200,
        name="hyst",
        params={"upper": 0.5, "lower": -0.5, "high": 1.0, "low": -1.0},
    )

    # Deadband block
    builder.add_block("Deadband", 200, 300, name="dead", params={"start": -0.5, "end": 0.5})

    # Rate Limiter
    builder.add_block(
        "RateLimiter", 200, 400, name="rate", params={"rising_slew": 2.0, "falling_slew": -2.0}
    )

    # Scopes for each output
    builder.add_block(
        "Scope", 400, 100, name="scope_sat", params={"title": "Saturation", "labels": "saturated"}
    )
    builder.add_block(
        "Scope", 400, 200, name="scope_hyst", params={"title": "Hysteresis", "labels": "hysteresis"}
    )
    builder.add_block(
        "Scope", 400, 300, name="scope_dead", params={"title": "Deadband", "labels": "deadband"}
    )
    builder.add_block(
        "Scope",
        400,
        400,
        name="scope_rate",
        params={"title": "Rate Limiter", "labels": "rate_limited"},
    )
