...
def create_pid_control_example():
    ...
    builder.add_block(
        ...
        200,
        name="ref",
        params={"step_time": 1.0, "initial_value": 0.0, "final_value": 1.0},
    )

    # Error calculation (reference - feedback)
    builder.add_block("Sum", 180, 200, name="error", params={"sign": "+-"})

    # PID Controller
    builder.add_block(
        "PID", 300, 200, name="pid", params={"Kp": 5.0, "Ki": 2.0, "Kd": 0.5, "N": 20.0}
    )

    # Plant: First-order system G(s) = 1/(s+1)
    builder.add_block(
        "TranFn", 450, 200, name="plant", params={"numerator": [1.0], "denominator": [1.0, 1.0]}
    )

    # Scope to view output
    builder.add_block(
        "Scope", 600, 200, name="scope", params={"title": "PID Response", "labels": "output"}
    )
