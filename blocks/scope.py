...
class ScopeBlock(BaseBlock):
    ...
    def execute(self, time, inputs, params, **kwargs):
        ...
        if params.get("_init_start_", True):
            ...
            logger.debug(
                f"Scope {params.get('_name_', 'unknown')} initialized, vec_labels: {params['vec_labels']}"
            )
            ...
            new_sample = np.asarray(
                np.concatenate(combined_input) if combined_input else np.array([0.0]), dtype=float
            )
            ...
            if new_sample.size != expected_dim:
                ...
                if not isinstance(buf, np.ndarray) or buf.ndim != 1 or cur_len > buf.size:
                    buf = (
                        np.atleast_1d(params.get("vector", np.array([0.0])))
                        .astype(float)
                        .flatten()
                        .copy()
                    )
