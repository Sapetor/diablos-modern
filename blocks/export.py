    ...
    def execute(self, time, inputs, params, **kwargs):
        ...
        if params.get("_init_start_", True):
            ...
            if new_dim != params.get("vec_dim", new_dim):
                ...
                if (
                    not isinstance(_buf, np.ndarray)
                    or _len < 1
                    or _len > _buf.shape[0]
                    or _buf.shape[1:] != _row.shape
                ):
                    _buf = np.asarray(
                        params.get("vector", np.array([inputs[0]])), dtype=float
                    ).copy()
                    _len = _buf.shape[0]
                if _len >= _buf.shape[0]:
                    _grown = np.empty(
                        (max(_len + 1, _buf.shape[0] * 2),) + _buf.shape[1:], dtype=float
                    )
