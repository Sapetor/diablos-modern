...
def _has_default_username(block: Any) -> bool:
    """True when ``block.username`` is still a label the app generated itself.

    The three ways a Subsystem comes into being each leave a different default:
    creating one from a selection gives ``username="Subsystem"`` beside
    ``name="Subsystem3"``, pasting gives ``"Subsystem3"`` beside ``"subsystem3"``
    (DBlock lower-cases ``name``), and loading restores whatever was saved. An
    exact ``username == name`` test matched none of them, so a masked subsystem
    never showed its mask name. ``name`` is deliberately left alone: it is the
    identifier connections and saved diagrams refer to. Any of the generated
    spellings counts instead, compared case-insensitively; a label the user
    typed is none of these and is kept.
    """
...
def set_mask(block: Any, mask: Optional[Dict[str, Any]], seed_values: bool = True) -> None:
    ...
    # A label still showing the previous mask's name was generated by that mask,
    # not typed by the user, so renaming the mask must carry it along.
    previous_label = previous.get("name")
    showing_previous = bool(previous_label) and getattr(block, "username", "") == previous_label
    apply_mask_appearance(block, force=showing_previous)
