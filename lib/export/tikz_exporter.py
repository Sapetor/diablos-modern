...
class TikZExporter:
    ...
    def export_snippet(self, options: Optional[Dict] = None) -> str:
        ...
        avg_gap = sum(self._layout_gaps) / len(self._layout_gaps) if self._layout_gaps else 2.5
    ...
    def _compute_textbook_layout(self, rendered_blocks, opts):
        ...
        roots = [b.name for b in rendered_blocks if b.name not in has_incoming]
        if not roots:
    ...
    def _output_continuation(self, rendered, visible_lines, opts):
        ...
        last_fb = max(feedback_sources, key=lambda n: self._block_order.get(n, 0))
    ...
    def _block_to_node(self, block, opts):
        ...
        if (
            opts.get("show_usernames", True)
            and block.username
            and block.username != block.name
    ...
    def _line_to_tikz_draw(self, line, opts):
        ...
        if dst_is_arrow:
            ...
            if getattr(self, "_branch_source", None) and src_block.name == self._branch_source:
                return None
            label_node = ""
            if label_text:
                label_node = f" node[midway, above, font=\\small] {{{label_text}}}"
            arrow_len = self._arrow_len
            return f"  \\draw[{style}] ({src_id}.{src_anchor}) --{label_node} +({arrow_len},0);"
        ...
        if not is_feedback:
            ...
            if label_text:
                label_node = f" node[midway, above, font=\\small] {{{label_text}}}"
            return (
                f"  \\draw[{style}] ({src_id}.{src_anchor}) --{label_node} ({dst_id}.{dst_anchor});"
            )
        ...
        if (
            getattr(self, "_branch_source", None)
        ...
        label_below = ""
        if fb_label_text:
            label_below = f" node[pos=0.5, below, font=\\small] {{{fb_label_text}}}"
