...
class LaTeXExporter:
    ...
    def matrix_to_latex(self, M: np.ndarray, name: str = "A", precision: int = 4) -> str:
        """
    ...
    def transfer_function_to_latex(
        self, num: List[float], den: List[float], name: str = "G"
    ) -> str:
        """
    ...
    def state_space_to_latex(
        self, A: np.ndarray, B: np.ndarray, C: np.ndarray, D: np.ndarray, precision: int = 4
    ) -> str:
        """
    ...
    def export_equations_only(self, equations: Dict[str, str], filename: str = None) -> str:
        """
...
class MathematicaExporter:
    ...
    def transfer_function_to_mathematica(self, num: List[float], den: List[float]) -> str:
        """
    ...
    def state_space_to_mathematica(
        ...
        def array_to_mma(arr):
            ...
            return "{" + ", ".join("{" + ", ".join(str(x) for x in row) + "}" for row in arr) + "}"
