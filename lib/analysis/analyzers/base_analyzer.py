    ...
    def _extract_system_model(self, block, canvas, visited=None):
        ...
        if "numerator" in params and "denominator" in params:
            try:
                ...
                logger.warning(
                    "Failed to extract transfer-function model from numerator/denominator params",
                    exc_info=True,
                )
                return None
        ...
        if "A" in params and "B" in params and "C" in params and "D" in params:
            try:
                ...
                logger.warning(
                    "Failed to extract state-space model from A/B/C/D params", exc_info=True
                )
                return None
        ...
        if block.block_fn == "PID":
            try:
                ...
                if sampling_time > 0.0:
                    num_d, den_d, dt_d = signal.cont2discrete((num, den), sampling_time)
                ...
                logger.warning(
                    "Failed to extract transfer-function model from PID params", exc_info=True
                )
                return None
        ...
        for coeffs in (num, den):
            if coeffs.size > 1 and np.any(coeffs):
                try:
                    ...
                    logger.debug(
                        "Failed to compute polynomial roots for frequency range; skipping these coeffs",
                        exc_info=True,
                    )
