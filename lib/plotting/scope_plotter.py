...
class ScopePlotter(_FieldScopeRenderMixin, _PyQtGraphScopeMixin):
    ...
    def _register_figure(self, fig):
        ...
        try:
            fig.canvas.mpl_connect("close_event", lambda event, f=fig: self._on_figure_closed(f))
        except Exception:
    ...
    def _close_plotty(self):
        ...
        if self.plotty is not None:
            try:
                ...
                logger.debug(
                    "No destroyed signal to disconnect on previous plot window", exc_info=True
                )
            try:
                self.plotty.close()
    ...
    def plot_again(self):
        ...
        try:
            ...
            logger.info(
                f"PLOT DEBUG: Engine={has_engine}, UseActive={use_active}, Blocks={len(source_blocks)}"
            )
            ...
            for x in source_blocks:
                if x.block_fn == "Scope":
                    ...
                    vectors.append(vec)
            valid_vectors = [
                v for v in vectors if v is not None and hasattr(v, "__len__") and len(v) > 0
            ]
            if valid_vectors:
                self.pyqtPlotScope()
                ...
                if block.block_fn == "FieldScope":
                    ...
                    if has_data:
                        try:
                            ...
                            logger.error(
                                f"PLOT ERROR: FieldScope {block.name} plotting failed: {e}"
                            )
                            import traceback
                ...
                if block.block_fn == "AgentScope":
                    ...
                    if has_data:
                        try:
                            ...
                            logger.error(
                                f"PLOT ERROR: AgentScope {block.name} plotting failed: {e}"
                            )
                            import traceback
                ...
                if block.block_fn == "FieldScope2D":
                    ...
                    if has_data:
                        try:
                            ...
                            logger.error(
                                f"PLOT ERROR: FieldScope2D {block.name} plotting failed: {e}"
                            )
                            import traceback
            ...
            if block.block_fn == "Scope":
                ...
                step_mode = any(
                    self._is_discrete_upstream(
                        ...
                        else getattr(conn, "srcblock", None)
                    )
                    for conn in inputs
                    if (
                        conn.get("srcblock")
    ...
    def get_scope_traces(self):
        ...
        """
        if (
            not hasattr(self.dsim, "timeline")
            or self.dsim.timeline is None
            or len(self.dsim.timeline) == 0
        ):
            return None, []
    ...
    def _print_verification_summary(self, source_blocks):
        ...
        for block in source_blocks:
            ...
            is_comparison = (
                n_signals == 2
                or "comparison" in block.name.lower()
                or "error" in block.name.lower()
            )
