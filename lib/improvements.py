...
class ValidationHelper:
    ...
    @staticmethod
    def validate_block_connections(
        blocks_list: List[Any], line_list: List[Any]
    ) -> Tuple[bool, List[str]]:
...
class LoggingHelper:
    ...
    def log_simulation_end(dsim_instance: Any, duration: float) -> None:
        ...
        if hasattr(dsim_instance, "time_step") and hasattr(dsim_instance, "sim_dt"):
            steps = (
                int(dsim_instance.time_step / dsim_instance.sim_dt)
                if dsim_instance.sim_dt > 0
                else 0
            )
