...
def build_waveequation1d(ctx):
    ...
    def exec_wave1d(
        ...
        du_dt, dv_dt = wave_rhs_1d(
            u, v, _c, _damping, _dx, force, _bc_type_left, bc_left, _bc_type_right, bc_right
        )
...
def build_advectionequation1d(ctx):
    ...
    def exec_advection1d(
        ...
        _dx=dx,
        _bc_type=bc_type,
        _inlet_key=inlet_key,
    ):
        c = y[_start : _start + _N]

        c_inlet = signals.get(_inlet_key, 0.0) if _inlet_key else 0.0

        dc_dt = advection_rhs_1d(c, _v, _dx, c_inlet, _bc_type, boundary_mode="penalty")
