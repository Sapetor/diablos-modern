...
def build_demux(ctx):
    ...
    def exec_demux(t, y, dy_vec, signals, _src=src, _shape=output_shape, _n=n_outputs):
...
def build_logicaloperator(ctx):
    ...
    if "_inputs_" in params:
        ...
        n_in = 1
    n_in = max(n_in, 1)
    baked_srcs = [input_sources[i] if i < len(input_sources) else None for i in range(n_in)]
