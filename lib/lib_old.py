...
class TkWidget:
    ...
    def __init__(self, name, params, external=False):
        ...
        self.top = tk.Toplevel(self.master)  # Create a new top-level window
        self.top.title(name+' parameters')
        self.entry_widgets = [self.create_entry_widget(x) for x in range(self.n)]

        self.external_toggle()

        tk.Button(self.top, text='Ok', command=self.top.quit).grid(row=self.n+1, column=0, sticky=tk.W, pady=4)
        self.top.mainloop()
