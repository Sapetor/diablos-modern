...
class DiagramBuilder:
    ...
    def add_block(
        ...
        """
        defaults = BLOCK_DEFAULTS.get(
            block_type,
            {"category": "Other", "color": "#808080", "b_type": 2, "in_ports": 1, "out_ports": 1},
        )
    ...
    def connect(self, src_block: str, src_port: int, dst_block: str, dst_port: int) -> str:
        """
...
def create_platoon_diagram(
    ...
    ref = builder.add_block(
        "Step", x=50, y=300, name="ref", params={"start_time": 0.0, "h_start": 0.0, "h_final": 10.0}
    )
    ...
    for i in range(n_vehicles):
        ...
        if i == 0:
            ...
            err = builder.add_block(
                "Sum", x=160, y=y, name=f"err{i + 1}", params={"sign": "++-"}, in_ports=3
            )
        else:
            err = builder.add_block(
                "Sum", x=160, y=y, name=f"err{i + 1}", params={"sign": "+-"}, in_ports=2
            )

        K = builder.add_block("Gain", x=230, y=y, name=f"K{i + 1}", params={"gain": 2.0})
        vel = builder.add_block(
            "Integrator",
            x=310,
            y=y,
            name=f"vel{i + 1}",
