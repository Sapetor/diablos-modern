...
|-------|-------------|----------------|
| **Step** | Unit step signal | `amplitude`, `start_time` |
| **Ramp** | Linear ramp signal | `slope`, `start_time` |
| **Sine** | Sinusoidal signal | `amplitude`, `frequency`, `phase` |
...
| **Switch** | Select input based on control | `threshold`, `n_inputs` |
