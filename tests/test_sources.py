...
def test_sine_block():
    ...
    assert np.isclose(result[0], 1.0, atol=1e-10), (
        f"Sine with phase at t=0: expected 1.0, got {result[0]}"
    )
...
def test_ramp_block():
    ...
    assert np.isclose(result[0], 1.0), (
        f"Ramp at t=1.5 with delay 0.5: expected 1.0, got {result[0]}"
    )
...
def test_constant_block():
    ...
    assert np.allclose(result[0], expected), (
        f"Constant vector: expected {expected}, got {result[0]}"
    )
