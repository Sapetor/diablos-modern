...
BLOX_INSTALLED = (
    bool(subprocess.run(["kpsewhich", "blox.sty"], capture_output=True, timeout=10).stdout.strip())
...
def serial_chain_diagram():
    ...
    step = make_block("Step", sid=0, category="Sources", left=0)
    gain = make_block("Gain", sid=1, category="Math", params={"gain": 5.0}, left=100)
    tf = make_block(
        "TranFn",
        sid=2,
        category="Control",
        params={"numerator": [1.0], "denominator": [1.0, 2.0, 1.0]},
        left=200,
    )
