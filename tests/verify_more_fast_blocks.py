...
def verify_deadband():
    ...
    lines = [make_line(b_sin, 0, b_db, 0), make_line(b_db, 0, b_scope, 0)]
...
def verify_exponential():
    ...
    lines = [make_line(b_ramp, 0, b_exp, 0), make_line(b_exp, 0, b_scope, 0)]

    model = type(
        "MockModel",
        (),
        {"blocks_list": blocks, "lines_list": lines, "link_goto_from": lambda: None},
    )()
    engine = SimulationEngine(model)
    engine.initialize_execution = lambda x: True
...
def verify_pid_derivative():
    ...
    lines = [make_line(b_ramp, 0, b_pid, 0), make_line(b_pid, 0, b_scope, 0)]
...
def verify_ratelimiter():
    ...
    lines = [make_line(b_step, 0, b_rl, 0), make_line(b_rl, 0, b_scope, 0)]
...
def verify_tranfn_casing():
    ...
    lines = [make_line(b_step, 0, b_tf, 0), make_line(b_tf, 0, b_scope, 0)]
