...
def test_integrator_block():
    ...
    final_value = params.get("mem", [0])[0]
    assert np.isclose(final_value, 0.5, rtol=0.1), (
        f"Integrator ramp: expected ~0.5, got {final_value}"
    )
...
def test_transfer_function_block():
    ...
    params = {"numerator": [0.01], "denominator": [1.0, -1.0], "_init_start_": True, "dtime": 0.01}
...
def test_exponential_block():
    ...
    params = {"a": 2.0, "b": 1.0}
    result = block.execute(0, {0: np.array([1.0])}, params)
    assert np.isclose(result[0], 2 * np.e, rtol=0.01), (
        f"2*Exp(1): expected {2 * np.e}, got {result[0]}"
    )
