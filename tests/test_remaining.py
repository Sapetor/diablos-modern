...
def test_transfer_function_block():
    ...
    params = {"numerator": [1.0], "denominator": [1.0, 1.0], "_init_start_": True, "dtime": 0.01}
...
def test_transport_delay_block():
    ...
    assert np.isclose(result[0], 5.0, rtol=0.1), (
        f"TransportDelay after: expected ~5.0, got {result[0]}"
    )
...
def test_export_block():
    ...
    assert "_export_data_" in params or "vector" in params or result is not None, (
        "Export should store data"
    )
