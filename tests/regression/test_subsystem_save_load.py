...
class TestSubsystemSaveLoad:
    ...
    def _build_diagram_with_subsystem(self, model):
        ...
        subsys = Subsystem(
            block_name="Subsystem1", sid=1, coords=QRect(250, 80, 100, 80), color=step.b_color
        )
        subsys.io_edit = "none"
        ...
        gain = DBlock(
            block_fn="Gain",
            sid=0,
            coords=QRect(280, 100, 60, 40),
            color=gain_mb.b_color,
            in_ports=1,
            out_ports=1,
            b_type=2,
            io_edit=gain_mb.io_edit,
            fn_name=gain_mb.fn_name,
            params={"gain": 2.0},
            block_class=gain_mb.block_class,
            colors=model.colors,
        )
        ...
        line_in = DLine(0, step.name, 0, subsys.name, 0, [QPoint(0, 0), QPoint(10, 10)])
        line_out = DLine(1, subsys.name, 0, scope.name, 0, [QPoint(0, 0), QPoint(10, 10)])
        model.line_list.extend([line_in, line_out])

        return subsys, gain

    def test_round_trip_preserves_subsystem_children(
        self, file_service, simulation_model, tmp_path
    ):
        ...
        reloaded = next(
            (b for b in simulation_model.blocks_list if b.block_fn == "Subsystem"), None
        )
        ...
        assert reloaded.ports == original_ports, "external ports layout did not survive save/load"
    ...
    def test_round_trip_flattens_to_primitives(self, file_service, simulation_model, tmp_path):
        ...
        flat_blocks, _ = Flattener().flatten(
            simulation_model.blocks_list, simulation_model.line_list
        )
