...
def create_large_diagram(num_chains=10, chain_length=5):
    ...
    for chain in range(num_chains):
        ...
        builder.add_block(
            ...
            y_base,
            name=src_name,
            params={"amplitude": 1.0, "frequency": 0.5 + chain * 0.1, "phase": 0.0},
        )
        ...
        for i in range(chain_length):
            ...
            if i % 3 == 0:
                ...
                builder.add_block("Gain", x, y_base, name=block_name, params={"gain": 1.1})
            elif i % 3 == 1:
                block_name = f"int_{chain}_{i}"
                builder.add_block(
                    "Integrator", x, y_base, name=block_name, params={"init_conds": 0.0}
                )
            else:
                ...
                builder.add_block(
                    ...
                    y_base,
                    name=block_name,
                    params={"upper_limit": 10.0, "lower_limit": -10.0},
                )
        ...
        builder.add_block(
            "Scope",
            50 + (chain_length + 1) * x_spacing,
            y_base,
            name=scope_name,
            params={"title": f"Chain {chain}", "labels": f"output_{chain}"},
        )
...
def run_benchmark():
    ...
    blocks_to_test = [
        (
            "Integrator",
            {"init_conds": 0.0, "method": "FWD_EULER", "_init_start_": True, "dtime": 0.01},
        ),
        ("Gain", {"gain": 2.0}),
        ("Sum", {"sign": "++"}, 2),
