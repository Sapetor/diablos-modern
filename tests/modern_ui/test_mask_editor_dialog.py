...
class TestMaskEditorDialog:
    def test_blank_mask_for_an_unmasked_subsystem(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
    ...
    def test_existing_mask_is_loaded_into_the_tabs(self, qapp, subsystem):
    ...
    def test_add_edit_and_read_back_a_parameter(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
    ...
    def test_list_and_choice_parameters(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
    ...
    def test_an_unparseable_default_is_kept_as_an_expression(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
            dialog.name_edit.setText("Plant")
            row = dialog.add_parameter()
    ...
    def test_remove_and_reorder(self, qapp, subsystem):
        set_mask(subsystem, VEHICLE_MASK)
        dialog = MaskEditorDialog(block=subsystem)
        try:
    ...
    def test_invalid_mask_keeps_the_dialog_open(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
            dialog.name_edit.setText("Plant")
            row = dialog.add_parameter()
    ...
    def test_accept_closes_on_a_valid_mask(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem)
        try:
            dialog.name_edit.setText("Plant")
            dialog.accept()
    ...
    def test_round_trip_through_set_mask(self, qapp, subsystem):
        dialog = MaskEditorDialog(block=subsystem, mask=VEHICLE_MASK)
        try:
