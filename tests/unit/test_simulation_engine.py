...
class TestGetNeighbors:
    ...
    def test_get_neighbors_returns_empty_for_unconnected_block(
        self, simulation_engine, simulation_model, sample_block
    ):
    ...
    def test_get_neighbors_returns_inputs(self, simulation_engine, simulation_model):
        ...
        line = DLine(0, block1.name, 0, block2.name, 0, [block1.out_coords[0], block2.in_coords[0]])
...
class TestGetOutputs:
    ...
    def test_get_outputs_returns_empty_for_unconnected_block(
        self, simulation_engine, simulation_model, sample_block
    ):
    ...
    def test_get_outputs_returns_multiple_connections(self, simulation_engine, simulation_model):
        ...
        line1 = DLine(0, source.name, 0, dest1.name, 0, [source.out_coords[0], dest1.in_coords[0]])
        line2 = DLine(1, source.name, 1, dest2.name, 0, [source.out_coords[1], dest2.in_coords[0]])
...
class TestCheckDiagramIntegrity:
    ...
    def test_check_diagram_integrity_passes_for_valid_diagram(
        self, simulation_engine, simulation_model
    ):
        ...
        line = DLine(0, source.name, 0, sink.name, 0, [source.out_coords[0], sink.in_coords[0]])
        simulation_model.line_list.append(line)

        result = simulation_engine.check_diagram_integrity()
        assert result == True

    def test_check_diagram_integrity_fails_for_unconnected_input(
        self, simulation_engine, simulation_model
    ):
    ...
    def test_check_diagram_integrity_fails_for_unconnected_output(
        self, simulation_engine, simulation_model
    ):
        ...
        line = DLine(0, source.name, 0, block.name, 0, [source.out_coords[0], block.in_coords[0]])
        simulation_model.line_list.append(line)
...
class TestGetMaxHierarchy:
    ...
    def test_get_max_hierarchy_returns_minus_one_for_uninitialized(
        self, simulation_engine, simulation_model, sample_block
    ):
